...
const MessageBubble = ({ data, sender, current }: { data: IMessages, sender: IFetchUser | undefined, current: IFetchCompany | undefined }) => {
  return (
    <View style={[styles.messageWrapper, { alignItems: data.sender == "company" ? "flex-start" : "flex-end" }]}>
      <View style={[styles.messageBox]}>
        <Text style={[styles.messageText, {color: data.sender == "company" ? Colors.metallicGreen : Colors.maastrichtBlue}]}>{data.message}</Text>
      </View>
